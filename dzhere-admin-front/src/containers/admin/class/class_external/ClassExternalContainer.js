...
import { setExternal, setCheck, setUserName } from "../../../../modules/admin/class/course";
...
const ClassExternalContainer = () => {
  ...
  async function classExternalDeleteApi() {
    console.log("외부장소 삭제");
    const data = await deleteExternal({ e_idx: classId, c_idx: selectClass, ag_idx: agency.ag_idx, u_name: name.trim().replace(" ", "") });
    if(data.length == 0){
      setName("");
