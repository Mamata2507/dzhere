...
import { getAgName, getClassList, getStudentList, setFilterList, deleteUser, addUser } from '../../../modules/admin/student/student'
...
const StudentListContainer = () => {
  ...
  const onAdd = () => {
    if(uPhone === '' || uName === '' || selectedClassAdd === 0){
      Alert.alert('빈 항목이 있습니다.');
    } else {
      let agIdx = agName.ag_idx
      dispatch(addUser({agIdx, uPhone, uName, selectedClassAdd}))
      Alert.alert('추가완료!');
      hideModalAdd()
      setSelectedClass(0)
      onChangeUname('')
