...
const TeacherAttendClassWebContainer = () => {
  ...
  const [removeModalButtonControl, setRemoveModalButtonControl] =
    useState(true);
  ...
  const checkHandler = (e) => {
    ...
    if (
      e.target.id !== "-999" &&
      !checkedList.includes(e.target.id) &&
      e.target.checked
    ) {
  ...
  const onChangeEndDate = (date) => {
    setEndDate(date);
    if(teacherIdxName.length !== 0 && endDate !== undefined && endDate !== null && startDate !== undefined && startDate !== null && agIdxTmp !== -1 && cIdxTmp !== -1){
      apiTeacherAttendList(teacherIdxName[0], startDate, endDate)
      .then(res => {
        if(res.result){
          console.log(
            "==================res.result==================",
