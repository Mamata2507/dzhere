...
public interface TeacherMapper {
    ...
    @Select("select A.a_idx, U.u_idx, U.u_phone, U.u_email, U.u_name, A.a_attend_time, A.a_exit_time, A.a_leave, A.a_late_status, A.a_absent, A.a_not_exit , A.a_today_date\n" +
            ...
            "where U.ag_idx=#{ag_idx} and U.u_accept=1 and U.u_auth = #{u_auth} and U.u_name=#{u_name} and a_today_date")
    List<Teacher> getTeacherSearchAll(int ag_idx, int c_idx, String u_name, int u_auth);

    @Select("select A.a_idx, U.u_idx, U.u_phone, U.u_email, U.u_name, A.a_attend_time, A.a_exit_time, A.a_leave, A.a_late_status, A.a_absent, A.a_not_exit , A.a_today_date\n" +
            "from User U join Attend A on U.u_idx = A.u_idx\n" +
            "where U.ag_idx=#{ag_idx} and U.u_accept=1 and U.u_auth = #{u_auth} and U.u_name=#{u_name} and a_late_status=#{a_late_status} and a_today_date")
    List<Teacher> getTeacherSearchAll_01(int ag_idx, int c_idx, String u_name, int u_auth, int a_late_status );

    @Select("select A.a_idx, U.u_idx, U.u_phone, U.u_email, U.u_name, A.a_attend_time, A.a_exit_time, A.a_leave, A.a_late_status, A.a_absent, A.a_not_exit , A.a_today_date\n" +
            "from User U join Attend A on U.u_idx = A.u_idx\n" +
            "where U.ag_idx=#{ag_idx} and U.u_accept=1 and U.u_auth = #{u_auth} and U.u_name=#{u_name} and a_absent=#{a_absent} and a_today_date")
    List<Teacher> getTeacherSearchAll_02(int ag_idx, int c_idx, String u_name, int u_auth, int a_absent);

    @Select("select A.a_idx, U.u_idx, U.u_phone, U.u_email, U.u_name, A.a_attend_time, A.a_exit_time, A.a_leave, A.a_late_status, A.a_absent, A.a_not_exit , A.a_today_date\n" +
            "from User U join Attend A on U.u_idx = A.u_idx\n" +
            "where U.ag_idx=#{ag_idx} and U.u_accept=1 and U.u_auth = #{u_auth} and U.u_name=#{u_name} and a_leave=#{a_leave} and a_today_date")
    List<Teacher> getTeacherSearchAll_03(int ag_idx, int c_idx, String u_name, int u_auth, int a_leave);

    @Select("select A.a_idx, U.u_idx, U.u_phone, U.u_email, U.u_name, A.a_attend_time, A.a_exit_time, A.a_leave, A.a_late_status, A.a_absent, A.a_not_exit , A.a_today_date\n" +
            "from User U join Attend A on U.u_idx = A.u_idx\n" +
            "where U.ag_idx=#{ag_idx} and U.u_accept=1 and U.u_auth = #{u_auth} and U.u_name=#{u_name} and a_not_exit=#{a_not_exit} and a_today_date")
    ...
    @Select("select A.a_idx, U.u_idx, U.u_phone, U.u_email, U.u_name, A.a_attend_time, A.a_exit_time, A.a_leave, A.a_late_status, A.a_absent, A.a_not_exit , A.a_today_date\n" +
            "from User U join Attend A on U.u_idx = A.u_idx\n" +
            "where U.ag_idx=#{ag_idx} and U.u_accept=1 and U.u_auth = #{u_auth} and U.u_name=#{u_name} and a_today_date between #{start_date} and #{end_date}")
    List<Teacher> getTeacherSearch(int ag_idx, int c_idx, String u_name, String start_date, int u_auth, String end_date);

    @Select("select A.a_idx, U.u_idx, U.u_phone, U.u_email, U.u_name, A.a_attend_time, A.a_exit_time, A.a_leave, A.a_late_status, A.a_absent, A.a_not_exit , A.a_today_date\n" +
            "from User U join Attend A on U.u_idx = A.u_idx\n" +
            "where U.ag_idx=#{ag_idx} and U.u_accept=1 and U.u_auth = #{u_auth} and U.u_name=#{u_name} and A.a_late_status = #{a_late_status} and A.a_today_date between #{start_date} and #{end_date}")
    ...
    @Select("select A.a_idx, U.u_idx, U.u_phone, U.u_email, U.u_name, A.a_attend_time, A.a_exit_time, A.a_leave, A.a_late_status, A.a_absent, A.a_not_exit , A.a_today_date\n" +
            "from User U join Attend A on U.u_idx = A.u_idx\n" +
            "where U.ag_idx=#{ag_idx} and U.u_accept=1 and U.u_auth = #{u_auth} and U.u_name=#{u_name} and A.a_absent = #{a_absent} and A.a_today_date between #{start_date} and #{end_date}")
    ...
    @Select("select A.a_idx, U.u_idx, U.u_phone, U.u_email, U.u_name, A.a_attend_time, A.a_exit_time, A.a_leave, A.a_late_status, A.a_absent, A.a_not_exit , A.a_today_date\n" +
            "from User U join Attend A on U.u_idx = A.u_idx\n" +
            "where U.ag_idx=#{ag_idx} and U.u_accept=1 and U.u_auth = #{u_auth} and U.u_name=#{u_name} and A.a_leave = #{a_leave} and A.a_today_date between #{start_date} and #{end_date}")
    ...
    @Select("select A.a_idx, U.u_idx, U.u_phone, U.u_email, U.u_name, A.a_attend_time, A.a_exit_time, A.a_leave, A.a_late_status, A.a_absent, A.a_not_exit , A.a_today_date\n" +
            "from User U join Attend A on U.u_idx = A.u_idx\n" +
            "where U.ag_idx=#{ag_idx} and U.u_accept=1 and U.u_auth = #{u_auth} and U.u_name=#{u_name} and A.a_not_exit = #{a_not_exit} and A.a_today_date between #{start_date} and #{end_date}")
